...
public class GroundIntakeRollerIOReal implements GroundIntakeRollerIO {
    ...
    public GroundIntakeRollerIOReal(CANBus rioCanbus, CANBus canivoreCanbus, StatusSignalCollection signal) {
        ...
        if (!status.isOK()) {
        new Alert("Ground Roller: Could not configure Motor 1. Error" + status.toString(), AlertType.kError)
            .set(true);
        }
